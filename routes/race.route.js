...
router.get("/:date/:index", async (req, res) => {
    try {
                ...
                return col.map((atrCell, index) => {
                    let horseNo =
                        race.horses.findIndex(
                            (horse) =>
                                horse.toLowerCase() === atrCell.toLowerCase()
                        ) + 1;
                    if (horseNo === 0) {
                        horseNo =
                            race.horses.findIndex(
        ...
        race.ATR2 = Array.isArray(race.ATR2)
            ? columnIndexer(race.ATR2)
            : race.ATR2;

        if (race.ATR2 && Array.isArray(race.ATR2)) {
            race.ATR2 = sortAsATR1(race.ATR2);
        }
        race.ATR3 = Array.isArray(race.ATR3)
            ? columnIndexer(race.ATR3)
            : race.ATR3;
        ...
        console.log(race.atrSelectedHorses)
        race.timeformSelectedHorses = catAdder(race.timeformSelectedHorses, "timeformFromAtr")
        race.sLifeSelectedHorses = catAdder(race.sLifeSelectedHorses, "timeformFromSLife")
